    ...
    if (isset($_FILES['image']['name'])) {
        ...
        if(!in_array(strtolower(fileext($_FILES['image']['name'])),$type))//判断上传文件的类型是否在范围内
        {
            ...
            if(is_uploaded_file($_FILES['image']['tmp_name']))
            {
                ...
                {
                    $imageUrl = $uploadfile;
                    $sql = "insert into lost (username,title,name,wupin,email,tel,address,place,content,create_time,imageUrl) values ('$username','$title','$chenghu','$wupin','$email','$tel','$address','$place','$content','$create_time','$imageUrl')";
                    if (mysqli_query($con,$sql)) {
                        echo <<<div
<div style="text-align: center;">失物招领发布成功！</div>
div;
                    }else{
                        echo <<<div
<center>失物招领发布失败！</center>
div;
                    }
                    echo <<<div
...
div;
                    echo <<<div
<br/><center><a href='javascript:history.go(-1)'>继续上传</a>|<a href='index.php'>返回失物招领</a></center>
div;
                }else
                {
                    echo <<<div
<div style="text-align: center;">失物招领发布失败！</div>
div;
                }
            }
        }
